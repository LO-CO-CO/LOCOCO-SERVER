...
public class Campaign extends BaseEntity {
	...
	public void validateParticipatableAt(Instant now) {
		if (applyStartDate != null && now.isBefore(applyStartDate)) {
			throw new CampaignNotStartedException();
		}
		if (applyDeadline != null && now.isAfter(applyDeadline)) {
			throw new CampaignExpiredException();
		}
		if (campaignStatus != CampaignStatus.RECRUITING) {
			throw new CampaignNotRecruitingException();
		}
		if (recruitmentNumber != null && recruitmentNumber > 0 && approvedNumber >= recruitmentNumber) {
			throw new CampaignRecruitmentFullException();
		}
	}

	public void approveByAdmin(Instant now) {
		if (campaignStatus != CampaignStatus.WAITING_APPROVAL) {
			throw new CampaignApprovalNotAllowedException();
		}

		if (applyStartDate != null && now.isBefore(applyStartDate)) {
			changeStatus(CampaignStatus.OPEN_RESERVED);
			return;
		}
		changeStatus(CampaignStatus.RECRUITING);
	}

	public void validateAdminModifiable() {
		if (campaignStatus != CampaignStatus.WAITING_APPROVAL) {
	...
	public void validateCapacityForApproval(int requestedApprovalCount) {
		if (approvedNumber + requestedApprovalCount > recruitmentNumber) {
